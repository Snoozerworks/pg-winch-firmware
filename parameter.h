...
const byte TACH0_ERR_COUNT = 3; // Number of samples with pump zero speed before triggering a tachometer error.
const byte TACH1_ERR_COUNT = 2; // Number of samples with drum zero speed before triggering a tachometer error.
const byte TACH1_ERR_SERVO_TRESHOLD = 200; // Max servo setpoint value allowed before resetting servo due to drum zero speed.
...
namespace Pins {
  ...
  DigitalPin CONFIG_UP_SW(5, INPUT_PULLUP); // Configuration mode up switch
  DigitalPin CONFIG_DN_SW(6, INPUT_PULLUP); // Configuration mode down switch
  DigitalPin CONFIG_ST_SW(7, INPUT_PULLUP); // Configuration mode set switch
